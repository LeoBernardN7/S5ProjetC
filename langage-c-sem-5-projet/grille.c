...
grille_t creer_grille(int largeur, int profondeur) {
    ...
    float** valeurs = malloc(sizeof(float*) * profondeur);
    for (int i=0; i<profondeur; i++) {
        valeurs[i] = malloc(sizeof(float) * largeur);
...
void detruire_grille(grille_t grille) {
    for (int i=0; i<grille.profondeur; i++) {
        free(grille.valeurs[i]);
    }
    free(grille.valeurs);
}
...
bool dans_les_bornes(grille_t grille, coord_t position) {
    return 
        0 <= get_x(position) && get_x(position) < get_largeur(grille) && 
        0 <= get_y(position) && get_y(position) < get_profondeur(grille)
    ;
}
...
coord_t superieur_droit(grille_t grille) {
    return creer_coord(get_largeur(grille) - 1, get_profondeur(grille) - 1);
}
...
void set_hauteur(grille_t grille, coord_t position, float hauteur) {
    grille.valeurs[position.y][position.x] = hauteur;
}
...
float get_hauteur(grille_t grille, coord_t position) {
    return grille.valeurs[position.y][position.x];
}
